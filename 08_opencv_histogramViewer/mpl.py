...
class BaseMplCanvas(FigureCanvas):
    ...
    def __init__(self, parent=None, width=5, height=4, dpi=72):
        ...
        self.axes.figure.canvas.mpl_connect('scroll_event', self.mouse_zoom)
    ...
    def mouse_zoom(self, event):
        ...
        # 増分の決定(upの場合と, dounの場合で-1をかける)
        dy = abs(yrange * 0.1) * (-1) ** (event.step < 0)

        if event.ydata > (yrange * 0.8 + ymin):
            self.axes.set_ylim(ymin, ymax + dy)
        elif event.ydata < (yrange * 0.2 + ymin):
            self.axes.set_ylim(ymin - dy, ymax)
        ...
        # 増分の決定(upの場合と, dounの場合で-1をかける)
        dx = abs(xrange * 0.1) * (-1) ** (event.step < 0)

        if event.xdata > (xrange * 0.8 + xmin):
            self.axes.set_xlim(xmin, xmax + dx)
        elif event.xdata < (xrange * 0.2 + xmin):
            self.axes.set_xlim(xmin - dx, xmax)
